...
// sentences that state a count and reports a failure, naming the line, on
// any mismatch.
//
// Exported as a function (rather than a script that only exits with a code)
// so test/inventory-elements.test.mjs can assert on structured results —
// including the vacuity floor — instead of re-parsing stdout. `npm test`
// runs that test file, which is what makes this run on every change instead
// of living in a name nobody types (20260906-054, review #3).
import { readFileSync } from 'node:fs';

export const MIN_ROWS = 45;

export function checkInventoryElements(path = 'docs/inventory-elements.md') {
  const text = readFileSync(path, 'utf8');
  const lines = text.split('\n');
  const failures = [];

  function fail(lineNo, msg) {
    failures.push({ line: lineNo + 1, msg });
  }

  // ── derive the tally from the per-component rows (the ground truth) ─────
  const blocks = text.split(/\n### `/).slice(1);
  if (blocks.length < MIN_ROWS) {
    return {
      vacuous: true,
      vacuousReason: `only ${blocks.length} component blocks parsed (floor is ${MIN_ROWS}) — refusing to verify against an empty-looking read`,
  ...
  const rows = blocks.map((b) => {
    ...
    if (!partsMatch || !slotMatch || !tokensMatch) {
      throw new Error(
        `block for \`${name}\` is missing a required field — parser or doc is broken`,
      );
    }
    return {
      name,
      parts: Number(partsMatch[1]),
      slot: slotMatch[1],
  ...
  if (!button || button.tokens === 0) {
    return {
      vacuous: true,
      vacuousReason:
        'button.css token count is 0 (or button row missing) — the token scan found nothing',
  ...
  // "...reads as 23 of\n52 (44%)..." never matches at all, which fails loudly
  // (an unmatched sentence is itself a failure below), not silently.
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (result.vacuous) {
    console.error(`VACUOUS: ${result.vacuousReason}`);
    process.exit(1);
  }
  if (result.failures.length > 0) {
    for (const f of result.failures) {
      console.error(`FAIL docs/inventory-elements.md:${f.line}: ${f.msg}`);
    }
    const { tally, mpTally, mpTotal } = result;
    console.error(
      `\nDerived tally: fixed ${tally.fixed} · n/a ${tally['n/a']} · free ${tally.free} · ambiguous ${tally.ambiguous} (of ${result.rowCount}); multi-part: fixed ${mpTally.fixed} · free ${mpTally.free} · ambiguous ${mpTally.ambiguous} (of ${mpTotal})`,
    );
    process.exit(1);
  }
  const { tally, mpTally, mpTotal, mpFixedPct } = result;
  console.log(
    `OK — ${result.rowCount} rows parsed; prose matches the row-derived tally: fixed ${tally.fixed} · n/a ${tally['n/a']} · free ${tally.free} · ambiguous ${tally.ambiguous}; multi-part 23-of-37-style: fixed ${mpTally.fixed} of ${mpTotal} (${mpFixedPct}%)`,
  );
}
