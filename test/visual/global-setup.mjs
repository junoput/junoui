...
export default function globalSetup() {
  ...
  process.stderr.write(
    [
      ...
      '',
      '  If the run dies with "Target page, context or browser has been closed",',
      '  read WHICH CALL THREW — the two cases print the same string:',
      '',
      '    browserType.launch:  the browser never started. Two causes here, and',
      '      the first version of this note named only one:',
      '        - the staged Chromium libraries are not in the environment',
      '          (LD_LIBRARY_PATH=~/.local/chromium-libs/extracted/usr/lib/x86_64-linux-gnu)',
      '        - or you forced the FULL chrome binary, which cannot launch on',
      '          this box even with those libs staged. Playwright uses',
      '          chrome-headless-shell for headless chromium, which is why the',
      '          suite works and why executablePath() naming the full binary',
      '          does not mean the full binary ran.',
      '    page.goto: / page.setContent:  the browser started and a page died.',
      '      Not a staging problem. Two known causes, check fontconfig first —',
      '      it reproduces every time on a quiet box, memory pressure did not:',
      '        - fontconfig cannot resolve a config. This box has no',
      '          /etc/fonts/fonts.conf and no fc-list, and when fontconfig finds',
      '          nothing Skia hits an unimplemented path on ANY text render —',
      '          not page-specific, not date-input-specific — and the renderer',
      '          dies with SIGTRAP.',
      '          SCOPE, because the first wording said "on this box" and that is',
      '          too broad: with LD_LIBRARY_PATH on the staged libs and',
      '          FONTCONFIG_FILE UNSET, a plain text page still rendered here.',
      '          The staged bundle ships its own libfontconfig.so.1 and its own',
      '          etc/fonts/fonts.conf, so that path can resolve without the',
      '          variable. Set it anyway — it is the configuration that is known',
      '          to work, and the one the memory note describes.',
      '          Fix: FONTCONFIG_FILE pointing at',
      '          a fonts.conf whose <dir> is the staged fonts path and whose',
      '          <cachedir> is writable (see the junoui-local-vr-setup memory).',
      '        - a renderer reaped under memory pressure looks identical in the',
      '          error string; check free memory if fontconfig is already wired',
      '          and this still happens. (20260918-002)',
      '',
